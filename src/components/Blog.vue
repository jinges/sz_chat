  ...
  <div class="blog" v-if="momentType!=4">
    ...
    <div class="content">
      ...
      <div v-if="momentType==2" class="images">
        <Pic v-for="(img, index) in fileList" :src="img" @click="showImg(img)" :key="index" />
      </div>
      <div class="footer">
        <div class="time">
          <!-- 2小时前 -->
        ...
        <el-popover ref="popover1" placement="left" trigger="click" style="opacity: 0.5;">
          <div>
            ...
            >
              <font-awesome-icon icon="comment-alt"></font-awesome-icon>
              评论
...
export default {
  ...
  computed: {
    ...
    likePeople() {
      var json = JSON.parse(this.likeCount);
      this.showLikeButton = !json
        .map(t => t.wechatId)
        .includes(util.getMyWxId());
    ...
    publisher() {
      return (
        this.$store.getters.filterFriendsById(this.wxid) || {
          name: "",
  ...
  methods: {
    ...
    showImg(url) {
      this.$msgbox({
        message:
          '<img style="max-width: 60vw;max-height: 90vh;" src="' + url + '"/>',
