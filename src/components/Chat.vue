<template>
  <div class="chat">
    ...
    <div class="footer">
      <div class="toolbar">
        ...
        <template v-if="uploadLoading">
          ...
          <span style="font-size: 12px;color: #A09D9D;">上传中</span>
        </template>
      </div>
      <AtMember
    ...
    <el-dialog
      width="1000px"
      center
      :title="isAddMember?'邀请成员':'移除成员'"
      :visible="showGroupMember"
      :before-close="closeGroupMember"
      destroy-on-close
    >
      <GroupMember
        ref="groupMember"
...
export default {
  ...
  methods: {
    ...
    onWsMsg: function(json) {
      ...
      //将之前发送中状态改为已发送
      if (
        json.messageType == "SERVER_TO_CLIENT" &&
        this.sendingMap[json.messageId]
      ) {
        ...
        for (var i = 0; i < this.history.length; i++) {
          if (
            this.history[i].sendingId == json.messageId &&
            this.history[i].sending == true
          ) {
            this.history[i].sending = false;
            //置顶会话
            if (
              json.commandName == "SERVER_SEND_TEXT_ACK" ||
              json.commandName == "SERVER_SEND_GROUP_TEXT_ACK"
            ) {
              //收到文本消息
              ...
              json.commandName == "SERVER_SEND_IMAGE_ACK" ||
              json.commandName == "SERVER_SEND_GROUP_IMAGE_ACK"
            ) {
              //收到图片消息
              this.$store.commit("topSession", {
                targetWxid: this.targetId,
              ...
              json.commandName == "SERVER_SEND_VOICE_ACK" ||
              json.commandName == "SERVER_SEND_GROUP_VOICE_ACK"
            ) {
              //收到语音消息
              this.$store.commit("topSession", {
                targetWxid: this.targetId,
              ...
              json.commandName == "SERVER_SEND_VIDEO_ACK" ||
              json.commandName == "SERVER_SEND_GROUP_VIDEO_ACK"
            ) {
              //收到视频消息
              this.$store.commit("topSession", {
                targetWxid: this.targetId,
              ...
              json.commandName == "SERVER_SEND_FILE_ACK" ||
              json.commandName == "SERVER_SEND_GROUP_FILE_ACK"
            ) {
              //收到文件消息
              this.$store.commit("topSession", {
                targetWxid: this.targetId,
              ...
              json.commandName == "SERVER_SEND_LINK_ACK" ||
              json.commandName == "SERVER_SEND_GROUP_LINK_ACK"
            ) {
              //收到链接消息
              this.$store.commit("topSession", {
                targetWxid: this.targetId,
        ...
        if (messageContent.targetWxid == this.targetId && !this.isGroup) {
          ...
          if (json.commandName == "CLIENTNOTIFY_TEXT") {
            ...
            this.history.push({
              ...
              time: new Date(
                parseInt(messageContent.createTime)
              ).toLocaleString()
            });
      ...
      } else if (json.messageType == "CLIENT_TO_SERVER") {
        //收到通知
        var messageContent = JSON.parse(json.messageContent);
        //当前聊天的通知
        if (messageContent.targetWxid == this.targetId && !this.isGroup) {
          //文本通知
          if (json.commandName == "CLIENT_SEND_TEXT") {
            // this.postMsgToHome(messageContent.sendContent);
            this.history.push({
              type: "receiver",
              media: "text",
          ...
          } else if (json.commandName == "CLIENT_SEND_IMAGE") {
            this.history.push({
              type: "receiver",
              media: "image",
              face: this.myFace,
          ...
          } else if (json.commandName == "CLIENT_SEND_VOICE") {
            this.history.push({
              type: "receiver",
              media: "voice",
              face: this.myFace,
          ...
          } else if (json.commandName == "CLIENT_SEND_VIDEO") {
            this.history.push({
              type: "receiver",
              media: "video",
              face: this.myFace,
          ...
          } else if (json.commandName == "CLIENT_SEND_FILE") {
            this.history.push({
              type: "receiver",
              media: "file",
              face: this.myFace,
          ...
          } else if (json.commandName == "CLIENT_SEND_LINK") {
            this.history.push({
              type: "receiver",
              media: "link",
              face: this.myFace,
          ...
          //文本通知
          if (json.commandName == "CLIENT_SEND_GROUP_TEXT") {
            this.history.push({
              type: "receiver",
              media: "text",
              face: this.groupMemberFace(messageContent.targetWxid),
          ...
          } else if (json.commandName == "CLIENT_SEND_GROUP_IMAGE") {
            this.history.push({
              type: "receiver",
              media: "image",
              face: this.groupMemberFace(messageContent.targetWxid),
          ...
          } else if (json.commandName == "CLIENT_SEND_GROUP_VOICE") {
            this.history.push({
              type: "receiver",
              media: "voice",
              face: this.groupMemberFace(messageContent.targetWxid),
          ...
          } else if (json.commandName == "CLIENT_SEND_GROUP_VIDEO") {
            this.history.push({
              type: "receiver",
              media: "video",
              face: this.groupMemberFace(messageContent.targetWxid),
          ...
          } else if (json.commandName == "CLIENT_SEND_GROUP_FILE") {
            this.history.push({
              type: "receiver",
              media: "file",
              face: this.groupMemberFace(messageContent.targetWxid),
          ...
          } else if (json.commandName == "CLIENT_SEND_GROUP_LINK") {
            this.history.push({
              type: "receiver",
              media: "link",
              face: this.groupMemberFace(messageContent.targetWxid),
    ...
    fileUploadSucc(res, file, fileList) {
      ...
      this.$axios
        .post(this.isGroup ? "/sendGroupFile" : "/sendFile", {
          imei: util.getImei(),
          fileType: fileType,
    ...
    sendTo: function() {
      ...
      if (!this.materialActiveName) {
        ...
        if (
          ...
          momentType = 2;
          apiType = "/sendFile";
        } else if (this.materialActiveName == "text") {
          momentType = 1;
          apiType = "/sendText";
